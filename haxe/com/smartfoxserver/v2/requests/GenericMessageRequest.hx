...
class GenericMessageRequest extends BaseRequest
{
	...
	private function validateBuddyMessage(sfs:SmartFox, errors:Array):Void
	{
		...
		var recipientId:Int=Std.parseFloat(_recipient)
		if(recipientId<0)
	...
	private function executeObjectMessage(sfs:SmartFox):Void
	{
		...
		if(Std.is(_recipient, Array))
		{
			...
			for(var item:Dynamic in potentialRecipients)
			{
				if(Std.is(item, User))
					recipients.set(item.id)
				else
					sfs.logger.warn("Bad recipient in DynamicMessage recipient list:" +(typeof item)+ ", expected type:User")
			}
