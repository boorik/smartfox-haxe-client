package com.smartfoxserver.v2.controllers
{
	...
	public class SystemController extends BaseController
	{
		...
		{
			var command:Object = requestHandlers[message.id]
		...
		*/
		private function fnUserLost(msg:IMessage):void
		{
			...
			if (user != null)
			{
				...
				var joinedRooms:Array = sfs.roomManager.getUserRooms(user)
				...
				for each (var room:Room in joinedRooms)
				{
		...
		private function fnInitBuddyList(msg:IMessage):void
		{
			...
			if (sfso.isNull(BaseRequest.KEY_ERROR_CODE))
			{
				...
				var buddyStates:Array = sfso.getUtfStringArray(InitBuddyListRequest.KEY_BUDDY_STATES)
		...
		private function fnGoOnline(msg:IMessage):void
		{
			...
			if (sfso.isNull(BaseRequest.KEY_ERROR_CODE))
			{
				...
				var buddy:Buddy = sfs.buddyManager.getBuddyByName(buddyName)
				var isItMe:Boolean = buddyName == sfs.mySelf.name
				var onlineValue:int = sfso.getByte(GoOnlineRequest.KEY_ONLINE)
				var onlineState:Boolean = onlineValue == BuddyOnlineState.ONLINE 
				
				var fireEvent:Boolean = true
		...
		*/
		private function fnSetUserPosition(msg:IMessage):void
		{
			...
			{
				for each (var uid:int in minusUserList)
				{
			...
			if (plusUserList != null)
			{
				for (i = 0; i < plusUserList.size(); i++)
				{
					...
					*/
					var userEntryPos:Array = encodedUser.getElementAt(5);
			...
			if (plusItemList != null)
			{
				for (i = 0; i < plusItemList.size(); i++)
				{
					...
					*/
					var itemEntryPos:Array = encodedItem.getElementAt(2);
